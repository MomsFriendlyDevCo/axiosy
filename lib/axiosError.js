...
export default class AxiosError extends Error {
	...
	*/
	...
	constructor(axiosError, ...params) {
		...
		this.url = this.axiosError.request?.url || this.axiosError.request.protocol + '//' + this.axiosError.request.host + this.axiosError.request.path;
	}
	...
	*/
	toString() {
		return this.axiosError?.response?.status >= 400 ? `Code ${this.axiosError.response.status} from ${this.url}`
